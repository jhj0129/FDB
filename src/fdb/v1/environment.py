...
class MujocoPushEnvironment:
    ...
    def _create_simulation(self, scenario: PhysicsScenario) -> tuple[Any, Any, int, int, int]:
        mujoco = _mujoco()
    ...
    def simulate(
        ...
        model, data, puck_id, puck_geom_id, floor_geom_id = self._create_simulation(scenario)
    ...
    def simulate_feedback(
        self, plan: FeedbackCandidate, scenario: PhysicsScenario | None = None
    ) -> PhysicsRollout:
        mujoco = _mujoco()
        scenario = scenario or PhysicsScenario()
        model, data, puck_id, puck_geom_id, floor_geom_id = self._create_simulation(scenario)
        ...
        stable = True
        out_of_bounds = False
        total_steps = plan.control_steps + plan.settle_steps

        for step in range(total_steps):
            data.xfrc_applied[puck_id, :] = 0.0
            if step < plan.control_steps:
                position = data.body("puck").xpos[:2].copy()
                velocity = [
                    float(data.joint("puck_x").qvel[0]),
                    float(data.joint("puck_y").qvel[0]),
                ]
                raw_force = [
                    plan.kp * float(target[axis] - position[axis])
                    - plan.kd * velocity[axis]
                    for axis in range(2)
                ]
                magnitude = math.hypot(*raw_force)
                scale = min(1.0, plan.max_force_newtons / max(magnitude, 1e-12))
                data.xfrc_applied[puck_id, :2] = [value * scale for value in raw_force]
            mujoco.mj_step(model, data)
            current = data.body("puck").xpos[:2].copy()
            path_length += float(math.hypot(*(current - prior)))
            prior = current
